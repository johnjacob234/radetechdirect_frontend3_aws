 ...
 function Ongoing() {
  ...
  let myOrder = filOrder.map(myorder => {
    ...
    <Grid item xs={12} sm={12} style={{textAlign:"left"}}><Typography variant="subtitle2"> Invoice # :<span style={{fontWeight:"bold",color:"#208769"}}> {myorder.orderID}</span></Typography></Grid>
          ...
          <Grid item xs={12} sm={12} style={{textAlign:"left"}}><Typography variant="subtitle2"> Balance : <span style={{fontWeight:"bold"}}> &#8369;{myorder.orderCustomerBalance.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")}.00</span></Typography></Grid>
          </Grid>
          <Divider style={{marginTop:"8px",marginBottom:"8px"}}/>
