...
let {managerStore:{listOfOrder,order,getOrderD}}=this.props;
let myId =JSON.parse(sessionStorage.getItem('userData'))
let filterComplete =listOfOrder.filter(order => {
   if(order.orderStatus === 'Complete' && order.dispatcher_ID === myId.account_ID || order.orderStatus ==='Complete' && order.packer_ID === myId.account_ID || order.orderStatus ==='Transfer' && order.packer_ID === myId.account_ID) {
       return order
   }
